...
def generate_obj_cistrans(imagetype, mean_value_offset=0):
    ...
    match imagetype:
        ...
        case "cistrans_little":
            ...
            alpha = 0.27
        case _:
            raise ValueError(f"imagetype {imagetype} not recognized")
    ...
    if imagetype in ["cistrans_little"]:
        ...
        ds_dark["sf"] = ds_dark.to_structurefactor("F_off", "PHI_off")
    else:
...
def generate_obj_v2(imagetype, kwargs={}):
    ...
    mean_value_offset = (
        0.3 if "mean_value_offset" not in kwargs.keys() else kwargs["mean_value_offset"]
    )
    if "offset" in imagetype:
        mean_value_offset = 90_000
        imagevariant = imagetype[:-7]
...
def struc2realspace(struc, hs_limit, noise_level=0, phi_noise=0.0):
    """
    ...
    # map_vals = cut_resolution(map_vals, low_resolution_limit=hs_limit)
    struc_vals = rs.DataSet(map_vals)
...
def pdb2realspace(pdb_file:str, hs_limit:float) -> np.ndarray:
    struc = gemmi.read_structure(pdb_file)
    return s2realspace(struc, hs_limit)

def s2realspace(struc, hs_limit:float) -> np.ndarray:
    density_map = gemmi.DensityCalculatorX()
    density_map.d_min = hs_limit
    density_map.rate = 1
    density_map.grid.setup_from(struc)
    for i, _ in enumerate(struc):
        density_map.put_model_density_on_grid(struc[i])

    return np.array(density_map.grid)

def generate_obj_cistrans_v6(
    occupancy, f_noise, phi_noise=0.0, no_negs=False, scaleit=True, hs_limit=1.
):
    # assert phi_noise is number
    assert isinstance(phi_noise, (int, float)), "phi_noise must be a number"
    dataloc = get_base_folder() + "/../synthetic_cistrans/"
    ...
    struc_light = gemmi.read_structure(dataloc + pdbname_light)
    struc_light = overwrite_occupancy(struc_light, occupancy)
    np_dark = struc2realspace(struc_dark, hs_limit)
    np_light = struc2realspace(struc_light, hs_limit)
    fou_dark = np.fft.fftn(np_dark)
    fou_light = np.fft.fftn(np_light)
    return np_dark, np_light, fou_dark, fou_light

def generate_obj_cistrans_v5(
    occupancy, f_noise, phi_noise=0.0, no_negs=False, scaleit=True, hs_limit=1.
):
    # assert phi_noise is number
    assert isinstance(phi_noise, (int, float)), "phi_noise must be a number"
    ...
    struc_light = gemmi.read_structure(dataloc + pdbname_light)
    struc_light = overwrite_occupancy(struc_light, occupancy)
    np_dark = s2realspace(struc_dark, hs_limit)
    np_light = s2realspace(struc_light, hs_limit)
    fou_dark = np.fft.fftn(np_dark)
    fou_light = np.fft.fftn(np_light)
    return np_dark, np_light, fou_dark, fou_light

def generate_obj_cistrans_v4(
    occupancy, f_noise, phi_noise=0.0, no_negs=False, scaleit=True, hs_limit=1.
):
    # assert phi_noise is number
    assert isinstance(phi_noise, (int, float)), "phi_noise must be a number"
    ...
    np_dark = s2realspace(struc_dark, hs_limit)
    np_light = s2realspace(struc_light, hs_limit)
    map_dark = rsmap.Map.from_3d_numpy_map(np_dark, spacegroup = struc_dark.find_spacegroup(),  cell=struc_dark.cell, high_resolution_limit=hs_limit)
    map_light = rsmap.Map.from_3d_numpy_map(np_light, spacegroup = struc_light.find_spacegroup(),  cell=struc_light.cell, high_resolution_limit=hs_limit)
    return map_dark, map_light
