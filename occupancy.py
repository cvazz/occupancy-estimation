...
def make_objs(
    ...
    i_diff = np.where(
        (x_pos < nx / 2) & (x_pos > nx / 2 - dx) &
        (y_pos < ny / 2) & (y_pos > ny / 2 - dy)
    )[0]
...
def make_working_vars(obj1,obj0, alpha, old_version=False):
    ...
    f0 = np.fft.fftn(obj0)
    f0_abs = np.abs(f0)
    f0_cphase = np.exp(1j * np.angle(f0))  # Equivalent to complex(cos(f0_phase), sin(f0_phase))
...
def make_f_xtr(alphas, f_dark, f_light, f_angle,version, noise_level=0, poisson_noise=-1):
    """
    ...
    f_xtr_abs = np.abs(
        2 / alphas[(slice(None), *many_none)] * (delta_f)[None, ...] + f_dark_abs[None, ...]
        )
    if poisson_noise > 0:
    ...
    match version:
        case 1:
            f_xtr = 2/alphas[(slice(None),*many_none)]*(delta_f_with_phase[None,...])+f_dark[None,...]
        case 2:
...
def get_neg_sum(f_xtrs):
    dens = np.array([np.fft.ifftn(f_xtr).real for f_xtr in f_xtrs])
    neg_sum = np.sum(dens[dens<0],axis=0)
    print(dens.shape, neg_sum.shape)
    return neg_sum*-1
...
def pandda(
    ...
    for ii, f_xtr in enumerate(f_xtrs):
        ...
        mean_global[ii] = pearsonr(rho_xtr[~mask_pks].flatten(), rho_dark[~mask_pks].flatten())[0]
        mean_global[ii] = pearsonr(rho_xtr.flatten(), rho_dark.flatten())[0]
        mean_local[ii] = pearsonr(rho_xtr[mask_pks].flatten(), rho_dark[mask_pks].flatten())[0]
...
def x8_density_map_inspect(f_xtrs, mask_pks, obj0, fofo):
    arrlen = len(f_xtrs)
    peak_sum = np.empty((arrlen))
    real_CC = np.empty((arrlen))
    real_CC2 = np.empty((arrlen))
    p1 = np.empty((arrlen))
    p2 = np.empty((arrlen))
    pos = np.empty((arrlen))
    neg = np.empty((arrlen))
    p_sum1 = np.empty((arrlen))
    p_sum2 = np.empty((arrlen))
    for ii, f_xtr in enumerate(f_xtrs):
        dens = np.fft.ifftn(f_xtr).real
        real_CC[ii] = pearsonr(dens.flatten(), fofo.flatten())[0]
        real_CC2[ii] = pearsonr(dens.flatten(), obj0.flatten())[0]
        p1[ii] = np.abs((dens-obj0)[mask_pks]).sum()
        p2[ii] = np.abs((dens)[mask_pks]).sum()
        p_sum1[ii] = np.abs(dens-obj0).sum()
        p_sum2[ii] = np.abs(dens).sum()
    return p1, p2, p_sum1, p_sum2
