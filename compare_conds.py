...
def get_intersect_and_angle(alpha_invs, neg_sum, n_largest):
...
def default_alpha_xtrs():
    ...
    alpha_xtrs = np.sort(np.concatenate([alpha_xtrs, alpha_xtrs_other,alpha_xtrs_other2 ]))[::-1]
    return alpha_xtrs
...
def main(alpha_xtrs, alpha_trues, noise_levels ):
    ...
    for noise_level in noise_levels:
        for alpha in alpha_trues: 
            ...
            intersection, angle = get_intersect_and_angle(alpha_invs, neg_sum, n_largest)
            marius_res = 2/intersection
            row = [alpha, noise_level, marius_res, panda_res,]
            rows.append(row)
            # print("hi")
